...
@allure.title("Test: Homepage Load After Login with {username}") 
def test_homepage_load_after_login(driver, username, password): 
    ...
    if username == "standard_user" and password == "secret_sauce":
        ...
        try:
            ...
            allure.attach(error_msg, name="Login Error Message", attachment_type=allure.attachment_type.TEXT)
            screenshot = driver.get_screenshot_as_png()
            allure.attach(screenshot, name="Login Failure Screenshot", attachment_type=allure.attachment_type.PNG)
            assert error_elem.is_displayed(), "Expected error message not shown."
        except NoSuchElementException:
            allure.attach(driver.get_screenshot_as_png(), name="No Error Message Screenshot", attachment_type=allure.attachment_type.PNG)
